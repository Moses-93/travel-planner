...
class CountPlacesError(TravelProjectError):
    ...
    def __init__(
        self, message: str = "A travel project cannot have more than 10 places."
    ) -> None:
...
class DuplicatePlaceError(TravelProjectError):
    ...
    def __init__(self, external_id: int) -> None:
        super().__init__(
            f"Place with external ID {external_id} is already in the project."
        )
...
class ProjectHasVisitedPlacesError(TravelProjectError):
    ...
    def __init__(
        self, message: str = "Cannot delete a project that has visited places."
    ) -> None:
        super().__init__(message)
